...
package org.acreetionos.launcher.preferences2
...
import org.acreetionos.launcher.data.Converters
...
import org.acreetionos.launcher.icons.CustomAdaptiveIconDrawable
import org.acreetionos.launcher.icons.shape.IconShape
import org.acreetionos.launcher.icons.shape.IconShapeManager
import org.acreetionos.launcher.predictions.PredictionMode
import org.acreetionos.launcher.preferences.PreferenceManager as LawnchairPreferenceManager
import org.acreetionos.launcher.qsb.providers.QsbSearchProvider
import org.acreetionos.launcher.search.algorithms.LawnchairSearchAlgorithm
import org.acreetionos.launcher.search.algorithms.engine.provider.web.WebSearchProvider
import org.acreetionos.launcher.smartspace.model.SmartspaceCalendar
import org.acreetionos.launcher.smartspace.model.SmartspaceMode
import org.acreetionos.launcher.smartspace.model.SmartspaceTimeFormat
import org.acreetionos.launcher.theme.color.ColorMode
import org.acreetionos.launcher.theme.color.ColorOption
import org.acreetionos.launcher.theme.color.ColorStyle
import org.acreetionos.launcher.ui.popup.LauncherOptionsPopup
import org.acreetionos.launcher.ui.popup.toOptionOrderString
import org.acreetionos.launcher.ui.preferences.components.HiddenAppsInSearch
...
import org.acreetionos.launcher.views.overlay.FullScreenOverlayMode
...
class PreferenceManager2 @Inject constructor(
    ...
    val acreetionPrivacyShield = preference(
        key = booleanPreferencesKey(name = "acreetion_privacy_shield"),
        defaultValue = true,
    )

    val acreetionZeroTelemetry = preference(
        key = booleanPreferencesKey(name = "acreetion_zero_telemetry"),
        defaultValue = true,
    )

    val acreetionSearchPrivacy = preference(
        key = booleanPreferencesKey(name = "acreetion_search_privacy"),
        defaultValue = true,
    )

    val acreetionRecentsPrivacy = preference(
        key = booleanPreferencesKey(name = "acreetion_recents_privacy"),
        defaultValue = false,
    )

    val acreetionClipboardGuard = preference(
        key = booleanPreferencesKey(name = "acreetion_clipboard_guard"),
        defaultValue = true,
    )

    val acreetionVpnSandbox = preference(
        key = booleanPreferencesKey(name = "acreetion_vpn_sandbox"),
        defaultValue = true,
        onSet = {
            org.acreetionos.launcher.search.privacy.AcreetionPrivacySandbox.isVpnProxyEnabled = it
        },
    )

    val acreetionDegoogledCore = preference(
        key = booleanPreferencesKey(name = "acreetion_degoogled_core"),
        defaultValue = true,
    )
